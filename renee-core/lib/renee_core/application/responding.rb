class ReneeCore
  class Application
    # Collection of useful methods for responding within a {ReneeCore} app.
    module Responding
      ...
      def respond(body=[], status=200, header={}, &blk)
        ReneeCore::Response.new(body, status, header).tap { |r| r.instance_eval(&blk) if block_given? }.finish
      end
      ...
      def interpret_response(response)
        ...
        when Array   then
          ...
          when 2 then ReneeCore::Response.new(response[1], HTTP_CODES[response[0]] || response[0]).finish
          else raise "I don't know how to render #{response.inspect}"
          end
        when String  then ReneeCore::Response.new(response).finish
        when Integer then ReneeCore::Response.new("Status code #{response}", response).finish
      ...
      # @example
      #     r = ReneeCore.new { get { halt redirect '/index' } }
      ...
      # @param [Integer] code The HTTP code to use.
      # @example
      #     r = ReneeCore.new { get { redirect! '/index' } }
      #     r.call(Rack::MockResponse("/")) # => [302, {"Location" => "/index"}, []]
      def redirect!(path, code = 302)
